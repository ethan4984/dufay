...
int nvme_irq_handle(struct irq_state *state, struct anchor **private)
{
	...
	for (int i = 0; i < nvme_controller->nvme_queue_pair_cnt; i++) {
		if ((uint64_t)nvme_controller->nvme_queue_pair[i].irq ==
			state->vector) {
	...
	volatile struct nvme_completion *completion_queue =
		(void *)queue->completion_queue_paddr + 0xffff800000000000;
	volatile uint32_t *completion_doorbell =
		(void *)nvme_regs + queue->completion_doorbell_offset;
	struct nvme_queue_entry *queue_entry =
		(struct nvme_queue_entry *)(queue->queue_entry_paddr +
									0xffff800000000000) +
		completion_queue[queue->cq_head].cid;
	...
	*completion_doorbell = ++queue->cq_head;
	...
	if (!queue_entry->blocking)
		queue_entry->response = true;
	else {
		struct syscall_response response =
			SYSCALL4(SYSCALL_FUTEX,
					 (uintptr_t)&queue_entry->response - 0xffff800000000000,
					 FUTEX_WAKE, true, false);
		if (response.ret == -1)
			RETURN_ERROR;
	}
