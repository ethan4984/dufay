...
int futex(uintptr_t uaddr, int ops, int expected, int virtual) {
	...
	int ret = hash_table_search(&futex_table, &futex_paddr,	
	...
	} else if(futex == NULL) RETURN_ERROR;
...
SYSCALL_DEFINE4(futex, uintptr_t, uaddr, int, ops, uint32_t, expected, int, virtual, {
	return futex(uaddr, ops, expected, virtual);
})
