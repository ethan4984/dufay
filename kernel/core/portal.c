...
static int portal_handle_direct(struct portal*, struct portal_req*, struct portal_resp*);
static int portal_handle_anon(struct portal*, struct portal_req*,  struct portal_resp*);
static int portal_handle_share(struct portal*, struct portal_req*, struct portal_resp*);
static int portal_handle_cow(struct portal*, struct portal_req*,  struct portal_resp*);
static int portal_handle_sp(struct portal*, struct portal_req*, struct portal_resp*);
...
int portal_resolve_fault(uintptr_t faulting_address, uint64_t error_code) {
	...
	if((error_code & X86_FLAGS_P) == 0) if(portal_fault_anon(page_table,
				pmle, faulting_address) == -1) RETURN_ERROR;
	if(page_entry & PORTAL_MAP_COW) if(portal_fault_cow(page_table,
				pmle, faulting_address) == -1) RETURN_ERROR;
	if(page_entry & PORTAL_MAP_SHARE) if(portal_fault_share(page_table,
				pmle, faulting_address) == -1) RETURN_ERROR;
	if(page_entry & PORTAL_MAP_SP) if(portal_fault_sp(page_table,
				pmle, faulting_address) == -1) RETURN_ERROR;

	return 0;
}

static int portal_fault_anon(struct page_table *page_table, uint64_t*, uintptr_t addr) {
...
static int portal_fault_share(struct page_table *page_table, uint64_t *pmle, uintptr_t addr) {
	page_table;
	pmle;
	addr;
	RETURN_ERROR;
...
static int portal_fault_sp(struct page_table *page_table, uint64_t *pmle, uintptr_t addr) {
	page_table;
	pmle;
	addr;
	RETURN_ERROR;
...
static int portal_fault_cow(struct page_table *page_table, uint64_t *pmle, uintptr_t addr) {
	if(page_table == NULL || pmle == NULL) RETURN_ERROR;

	struct page *page = NULL; 
	int ret = hash_table_search(page_table->pages, &addr, sizeof(addr), (void**)page);
	if(unlikely(ret == -1 || page == NULL)) RETURN_ERROR;

	uint64_t original_frame = *pmle & ~(0xfff);
	uint64_t new_frame = ({
		uint64_t frame;
		if(*page->refcnt <= 1) frame = original_frame;
		else {
			page->frame = alloc(sizeof(struct frame));
			new_frame = pmm_alloc(1, 1);
			memcpy((void*)new_frame + HIGH_VMA, (void*)original_frame + HIGH_VMA, PAGE_SIZE);
		}
		frame;
	});
...
static int portal_handle_anon(struct portal *portal, struct portal_req *req, struct portal_resp *resp) {
	...
	for(size_t i = 0; i < req->morphology.pcnt; i++) {	
		...
		int ret = hash_table_push(portal->page_table->pages, &page->vaddr, page, sizeof(page->vaddr));
		if(ret == -1) RETURN_ERROR;
	}
...
static struct portal *portal_copy_tree(struct page_table *source_table,
	struct portal *root, uintptr_t base, size_t limit) {
	if(source_table == NULL || root == NULL) return NULL;
	
	if(base == -1 && limit == -1) {
		struct portal *region = alloc(sizeof(struct portal));
		if(unlikely(region == NULL)) return NULL;
		...
		*region = *root;
		...
		return region;
	} else if((base == -1 && limit != -1) ||
		(base != -1 && limit == -1)) return NULL;

	struct portal *src_portal = ({
		struct portal *root = source_table->portal_root;
		for(;root;) {
			if(root->base <= base && (root->base + root->limit) >= base) break;
		}
		root;
	});
	...
	*region = *src_portal;
	...
	if(region->limit == src_portal->limit || region->limit == limit) return region;
...
static int portal_handle_cow(struct portal *portal, struct portal_req *req, struct portal_resp*) {
	...
	if(req->cow.source.asid == -1) source_table = current_context->page_table;
	else {
		int ret = vmm_as_find(req->cow.source.asid, &source_table);
		if(ret == -1 || source_table == NULL) return -1;
	}
	...
	if((req->cow.source.base == -1 && req->cow.destination.base != -1) ||
		(req->cow.source.base != -1 && req->cow.destination.base == -1)) return -1;

	if(req->cow.source.base == -1 && req->cow.destination.base == -1) {
		for(int i = 0; i < source_table->pages->capacity; i++) {
			__label__ skip;
			struct page *src_page = source_table->pages->data[i]; 
			if(src_page == NULL) continue;
			if(src_page->frame == NULL) goto skip;
			if((*src_page->pmle & PORTAL_MAP_SHARE) != PORTAL_MAP_SHARE) {
				*src_page->pmle &= ~(X86_FLAGS_RW);
				*src_page->pmle |= PORTAL_MAP_COW;
				src_page->flags = (src_page->flags & ~(X86_FLAGS_RW)) | PORTAL_MAP_COW;
			}
skip:
			src_page->refcnt++;

			struct page *dest_page = alloc(sizeof(struct page));
			if(dest_page == NULL) RETURN_ERROR;
			*dest_page = *src_page;

			dest_page->pmle = destination_table->map_page(destination_table,
				dest_page->vaddr, dest_page->frame->paddr, dest_page->flags);
		...
		destination_table->portal_root = portal_copy_tree(source_table,
			source_table->portal_root, -1, -1);
		if(destination_table->portal_root == NULL) RETURN_ERROR;
	} else {
		...
		for(int i = 0; i < DIV_ROUNDUP(req->cow.limit, PAGE_SIZE); i++) {
			...
			ret = hash_table_search(source_table->pages, &src_vaddr, sizeof(src_vaddr), (void**)&src_page);
			if(src_page == NULL) continue; 
			if(src_page->frame == NULL) goto skip;
			if((*src_page->pmle & PORTAL_MAP_SHARE) != PORTAL_MAP_SHARE) {
				*src_page->pmle &= ~(X86_FLAGS_RW);
...
skip:
		...
		destination_table->portal_root = portal_copy_tree(source_table,
			source_table->portal_root, req->cow.source.base, req->cow.limit);
		if(destination_table->portal_root == NULL) RETURN_ERROR;
	}
...
void portal_destroy(struct portal *portal) {
	...
	for(int i = 0; i < portal->pages->capacity; i++) {
		...
		if(page) {
			if(--page->frame->refcnt == 0) pmm_free(page->frame->paddr, 1);
			if(--(*page->refcnt) == 0) free(page);
		}
	}
