...
static int elf64_read(struct elf64_file *file, void *buffer, int offset,
					  size_t cnt)
{
	if (unlikely(file->data.buffer == NULL || buffer == NULL))
		RETURN_ERROR;
	if (unlikely(file->data.length < offset))
		return 0;

	int data_to_read =
		(file->data.length < (offset + cnt)) ? file->data.length - offset : cnt;
	memcpy(buffer, file->data.buffer + offset, data_to_read);

	return data_to_read;
}

static int elf64_map(struct elf64_file *file, struct elf64_phdr *phdr,
					 struct elf64_shdr *shdr, bool place)
{
	...
	for (int j = 0; j < page_cnt; j++) {
		...
		file->page_table->map_page(file->page_table, virtual, physical[j],
								   page_flags);
	}
...
int elf64_file_init(struct elf64_file *file)
{
	...
	ret = elf64_read(file, file->shdr, file->hdr->shoff,
	...
	ret = elf64_read(file, file->phdr, file->hdr->phoff,
	...
	ret = elf64_read(file, (char *)file->strtab, file->strtab_hdr->sh_offset,
	...
	for (size_t i = 0; i < file->hdr->ph_num; i++) {
		...
		for (int j = 0; j < phdr->p_filesz / sizeof(struct elf64_dyn); j++) {
			...
			ret = elf64_read(file, &dyn,
							 phdr->p_offset + j * sizeof(struct elf64_dyn),
									...
									size_t section_offset)
{
	...
	uint64_t *ptr = (uint64_t *)(file->data.buffer + section_offset - vaddr +
								 relocation->r_offset);
	...
	case R_X86_64_64: {
		...
		int ret = elf64_read(file, &symtab,
		...
		if (symtab.st_shndx == ELF_SHN_UNDEF &&
			...
			*ptr = file->aslr_layout->lower_bound + symtab.st_value +
				   relocation->r_addend;
								  ...
								  size_t length, size_t section_offset)
{
	for (int offset = 0; offset < file->dynamic.pltrel_size;
		...
		struct elf64_rela relocation;
		int ret = elf64_read(file, &relocation, file->dynamic.jmprel + offset,
	...
	for (int offset = 0; offset < file->dynamic.rela_size;
		 offset += file->dynamic.rela_ent) {
		struct elf64_rela relocation;
		int ret = elf64_read(file, &relocation,
