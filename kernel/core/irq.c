...
int irq_cortex_resolve_fault(uintptr_t faulting_address, uint64_t error_code)
{
	...
	for (int i = 0; i < cortex_table.capacity; i++) {
		...
		if (faulting_address >= cortex->aslr_layout->lower_bound &&
			faulting_address < cortex->aslr_layout->upper_bound)
			goto found;
	}
	RETURN_ERROR;
found:
...
static int irq_cortex_instantiate(const char *path, const char *identifier,
								  int vector)
{
	if (identifier == NULL)
		RETURN_ERROR;
...
{
	if (identifier == NULL || anchor == NULL)
		RETURN_ERROR;
	...
	anchor->last = NULL;
	if (cortex->anchor_root)
		cortex->anchor_root->last = anchor;
	cortex->anchor_root = anchor;

	return 0;
}

SYSCALL_DEFINE3(irq_cortex_instantiate, const char *, path, const char *,
				identifier, int, vector, {
					int ret = irq_cortex_instantiate(path, identifier, vector);
					if (ret == -1)
						return -1;
				})

SYSCALL_DEFINE2(irq_cortex_anchor, const char *, identifier, struct anchor *,
				anchor, {
